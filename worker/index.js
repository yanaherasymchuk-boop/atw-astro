...
async function handleContact(request, env) {
  ...
  // Honeypot: bots filling this field get a fake success.
  ...
  const recipient =
    env.CONTACT_RECIPIENT || "atwcardetailing@gmail.com";

  const sender =
    env.CONTACT_SENDER || "website@atwdetailing.com";

  if (!accountId || !apiToken) {
    return json(
      {
        error:
          "Message service is not configured yet. Please text or email us instead.",
      },
      503
    );
  }
...
`;
  ...
  try {
    ...
    return json(
      {
        error:
          "Could not send message. Please text or email us instead.",
      },
      502
    );
  }
